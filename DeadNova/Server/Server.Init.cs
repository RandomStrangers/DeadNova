...
using System.Net;
using DeadNova.Commands.World;
using DeadNova.DB;
using DeadNova.Games;
using DeadNova.Generator;
...
namespace DeadNova {
    ...
    public sealed partial class Server {
        ...
        static void LoadMainLevel(SchedulerTask task) {
            try {
#if DEV_BUILD_NOVA
                mainLevel = LevelActions.Load(Player.Nova, Server.Config.MainLevel, false);
#else
...
#if DEV_BUILD_NOVA
            MapGen.Find("Flat").Generate(Player.Nova, mainLevel, "");
#else
...
#if DEV_BUILD_NOVA
            Player.Nova.DatabaseID = NameConverter.InvalidNameID("&7(&0Dead&5Nova&7)");
#else
...
#if DEV_BUILD_NOVA
                LevelActions.Load(Player.Nova, map, false);
#else
