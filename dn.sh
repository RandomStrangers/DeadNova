...
dndir="./"							##  Folder containing DeadNova.exe
...
gamename="RedCraft - DeadNova"		##  Arbitrary name of server, will not affect actual server name
gamepid="${dndir}/dn.pid"			##  If you do not know what this is, do not worry about it, for "silent" mode only.
gamelog="${dndir}/dn.log"			##  This logs everything sent to console, if started in "silent" mode
...
if [ ! -f "${dndir}/DeadNova.exe" ]; then
	echo -e "cannot find ${dndir}/DeadNova.exe!"
...
case "$1" in
	silent)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${dndir}/DeadNova.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			if ${monobin} ${gameopt} ${dndir}/DeadNova.exe 1>> ${gamelog} 2>&1 & sleep 3 ; then
				pid=`ps -ef |grep "${monobin} ${gameopt} ${dndir}/DeadNova.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game { print $2 } ; END {exit status}'`
				echo ${pid} > ${gamepid}
...
;;
	normal)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${dndir}/DeadNova.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			echo -e
			${monobin} ${gameopt} ${dndir}/DeadNova.exe
			if $autorestart ; then
				$0
			else
				exit 0
...
;;
	*)
		if [ -f "${dndir}/DeadNova_.update" ]; then
			if [ -f "${dndir}/DeadNova.update" ]; then
				echo
				echo Update Found!
				echo -n Applying update:
				rm ${dndir}/DeadNova.exe
				rm ${dndir}/DeadNova_.dll
				mv ${dndir}/DeadNova.update ${dndir}/DeadNova.exe
				mv ${dndir}/DeadNova_.update ${dndir}/DeadNova_.dll
				if [ -f "${dndir}/DeadNova_.update" ]; then
					if [ -f "${dndir}/DeadNova.update" ]; then
						echo -e FAILED!
						if [ -f "${dndir}/DeadNova_.dll" ]; then
							if [ -f "${dndir}/DeadNova.exe" ]; then
								$0 ${default}
							fi
						else
